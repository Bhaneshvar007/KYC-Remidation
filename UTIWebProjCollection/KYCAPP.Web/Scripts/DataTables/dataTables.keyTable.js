...
$.extend( KeyTable.prototype, {
	 ...
	 *
	 * @param  {string} state Can be `true`, `false` or `-string navigation-`
	 */
	...
	_focus: function ( row, column, shift, originalEvent )
	{
		...
		if ( typeof row !== 'number' ) {
			...
			// point, since `rows().indexes()` includes  rows that are
	...
	_key: function ( e )
	{
		...
		var navEnable = enable === true || enable === 'navigation-';
		if ( ! enable ) {
			return;
		}
		...
		switch( e.keyCode ) {
			...
			default:
				// Everything else - pass through  when fully enabled
				if ( enable === true ) {
	...
	_shift: function ( e, direction, keyBlurable )
	{
		...
		// When server-side processing, `rows().indexes()`  gives the rows
...
KeyTable.defaults = {
	 ...
	 * visible columns as  visible columns can gain focus.
	 * @type {String}
	 */
	columns: '', // all
	 ...
	 * Editor's inline editing. Can be `navigation-` or `tab-`.
	 * @type {String}
	 */
	editorKeys: 'navigation-',
