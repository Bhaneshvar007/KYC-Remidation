...
* You should not use this file at runtime inside the browser--it is 
* intended to be used  for design-time IntelliSense.  Please use the
...
* for informational purposes  and are not the license terms under
...
$.extend($.validator, {
	...
	prototype: {
		...
		numberOfInvalids: function() {
			...
			/// This depends on the internal validator state. It covers all fields  after
			/// validating the complete form (on submit or via $("form").valid()). After validating
			/// a single element,  that element is counted. Most useful in combination with the
		...
		elements: function() {
			...
			.filter(function() {
				...
				// select  the first element for each name, and  those with rules specified
...
// handler is  called when $(event.target).is(delegate), in the scope of the jquery-object for event.target 
;(function($) {
	//  implement if not provided by jQuery core (since 1.4)
