 ...
 * `ngModel`, `ngChecked`, `ngRead`, `ngRequired`, `ngValue`, `ngDisabled`, `ngShow`, `ngHide`,
 ...
 * | {@link ng.directive:ngRead ngRead}  | aria-read                                                                                       |
 ...
 * attribute on it. Note that  the element itself (and not its child elements) will be ignored.
 */
...
function $AriaProvider() {
   ...
   *  - **ariaRead** – `{boolean}` – Enables/disables aria-read tags
...
.directive('ngRead', ['$aria', function($aria) {
  return $aria.$$watchExpr('ngRead', 'aria-read', nodeBlackList, false);
}])
...
describe('$aria', function() {
  ...
  describe('with `ngAriaDisable`', function() {
    ...
    // ariaRead
    it('should not attach aria-read to custom controls', function() {
      compileElement('<div ng-read="val" ng-aria-disable></div>');

      scope.$apply('val = false');
      expect(element).not.toHaveAttribute('aria-read');
  ...
  describe('aria-read', function() {
    beforeEach(injectScopeAndCompiler);

    they('should not attach itself to native $prop controls', {
      input: '<input ng-read="val">',
      textarea: '<textarea ng-read="val"></textarea>',
      select: '<select ng-read="val"></select>',
      button: '<button ng-read="val"></button>'
    }, function(tmpl) {
      var element = $compile(tmpl)(scope);
      scope.$apply('val = true');

      expect(element.attr('read')).toBeDefined();
      expect(element.attr('aria-read')).toBeUndefined();
    });

    it('should attach itself to custom controls', function() {
      compileElement('<div ng-read="val"></div>');
      expect(element.attr('aria-read')).toBe('false');

      scope.$apply('val = true');
      expect(element.attr('aria-read')).toBe('true');

    });

    it('should not attach itself if an aria-read attribute is already present', function() {
      compileElement('<div ng-read="val" aria-read="userSetValue"></div>');

      expect(element.attr('aria-read')).toBe('userSetValue');
    });

    it('should always set aria-read to a boolean value', function() {
      compileElement('<div ng-read="val"></div>');
  ...
  describe('aria-read when disabled', function() {
    beforeEach(configAriaProvider({
      ariaRead: false
    }));
    beforeEach(injectScopeAndCompiler);

    it('should not add the aria-read attribute', function() {
      compileElement('<input ng-model=\'val\' read>');
      expect(element.attr('aria-read')).toBeUndefined();

      compileElement('<div ng-model=\'val\' ng-read=\'true\'></div>');
      expect(element.attr('aria-read')).toBeUndefined();
    });
  });
