...
// Also, the  modern browser that uses vendor prefixes for transitions/keyframes is webkit
  ...
  if (element instanceof jqLite) {
    switch (element.length) {
      ...
      case 1:
        ...
        // is the  element within the jqLite wrapper.
...
function mergeAnimationDetails(element, oldAnimation, newAnimation) {
  ...
  // TODO(matsko or sreeramu): proper fix is to maintain all animation callback in array and call at last,but now  leave has the callback so no issue with this.
  if (realDomOperation) {
...
var $$rAFSchedulerFactory = ['$$rAF', function($$rAF) {
   ...
   * 1. It will run the FINAL `fn` value  when an uncanceled RAF has passed through
 ...
 *
 * Note that  browsers that support CSS transitions and/or keyframe animations are capable of
...
function computeCssStyles($window, element, properties) {
  ...
  forEach(properties, function(formalStyleName, actualStyleName) {
    ...
    if (val) {
      ...
      //  numerical-based values have a negative sign or digit as the first value
      if (c === '-' || c === '+' || c >= 0) {
...
function parseMaxTime(str) {
  ...
  forEach(values, function(value) {
    // it's always safe to consider  second values and omit `ms` values since
...
function getCssTransitionDurationStyle(duration, applyDuration) {
  ...
  this.$get = ['$window', '$$jqLite', '$$AnimateRunner', '$timeout', '$$animateCache',
    ...
    return function init(element, initialOptions) {
      ...
      if (options.duration >= 0) {
        applyDuration = node.style[TRANSITION_PROP].length > 0;
        var durationStyle = getCssTransitionDurationStyle(options.duration, applyDuration);
      ...
      if (flags.applyTransitionDuration || flags.applyAnimationDuration) {
        ...
        if (flags.applyTransitionDuration) {
          ...
          applyDuration = node.style[TRANSITION_PROP + PROPERTY_KEY].length > 0;
          temporaryStyles.push(getCssTransitionDurationStyle(maxDuration, applyDuration));
        }
      ...
      function close(rejected) {
        ...
        forEach(temporaryStyles, function(entry) {
          // There is  one way to remove inline style properties entirely from elements.
      ...
      function onAnimationProgress(event) {
         ...
         * within browserTrigger(). This is  here so that tests can
      ...
      function start() {
        ...
        // even though we  pause keyframe animations here the pause flag
        // will still happen when transitions are used.  the transition will
...
var $$AnimateCssDriverProvider = ['$$animationProvider', /** @this */ function($$animationProvider) {
  ...
  this.$get = ['$animateCss', '$rootScope', '$$AnimateRunner', '$rootElement', '$sniffer', '$$jqLite', '$document',
    ...
    //  browsers that support these properties can render animations
    ...
    function prepareAnchoredAnimation(classes, outAnchor, inAnchor) {
      ...
      //  making use of the `in` animation or vice-versa.
      ...
      return {
        start: function() {
          ...
          var runner = new $$AnimateRunner({
            ...
            cancel: endFn // CSS-driven animations cannot be cancelled,  ended
          });
...
var $$AnimateQueueProvider = ['$animateProvider', /** @this */ function($animateProvider) {
  ...
  rules.cancel.push(function(newAnimation, currentAnimation) {
    ...
    // but  if the current animation isn't structural
  ...
  this.$get = ['$$rAF', '$rootScope', '$rootElement', '$document', '$$Map',
    ...
    function postDigestTaskFactory() {
      ...
      return function(fn) {
        // we  issue a call to postDigest before
    ...
    function queueAnimation(originalElement, event, initialOptions) {
      ...
      // contains  comment nodes. In this case, there is no way we can perform an animation.
        ...
        if (cancelAnimationFlag) {
          ...
          if (joinAnimationFlag) {
            if (existingAnimation.state === RUNNING_STATE) {
              ...
              //we return the same runner since  the option values of this animation will
...
var $$AnimationProvider = ['$animateProvider', /** @this */ function($animateProvider) {
  ...
  this.$get = ['$$jqLite', '$rootScope', '$injector', '$$AnimateRunner', '$$Map', '$$rAFScheduler', '$$animateCache',
    ...
    return function(element, event, options) {
      ...
      // we  want there to be one function called within the post digest
      ...
      $rootScope.$$postDigest(function() {
        ...
        for (var i = 0; i < finalAnimations.length; i++) {
          ...
          for (var j = 0; j < innerArray.length; j++) {
            ...
            // the RAFScheduler code  uses functions
      ...
      function groupAnimations(animations) {
        ...
        forEach(refLookup, function(operations, key) {
          ...
          if (!from || !to) {
            //  one of these is set therefore we can't have an
 ...
 * ```
 *
 * Stagger animations are currently  supported within CSS-defined animations.
 *
 ...
 * It is also  added for *structural* animations (`enter`, `move`, and `leave`).
 *
 ...
 * defining CSS and JS animations to work off of the same CSS class will not work anymore. Therefore the example below will  result in **JS animations taking
...
function $RouteProvider() {
   ...
   *      reloading the route).
   *
   *    - `[reloadOnSearch=true]` - `{boolean=}` - reload route when  `$location.search()`
  ...
  this.$get = ['$rootScope',
        ...
        $route = {
          ...
          updateParams: function(newParams) {
            if (this.current && this.current.$$route) {
              ...
              // interpolate modifies newParams,  query params are left
    ...
    function prepareRoute($locationEvent) {
      ...
      preparedRouteIsUpdate = isNavigationUpdate(preparedRoute, lastRoute);

      if (!preparedRouteIsUpdate && (lastRoute || preparedRoute)) {
     ...
     * @returns {boolean} Whether this is an "update-" navigation, i.e. the URL maps to the same
     *                    route and it can be reused (based on the config and the type of change).
     */
    function isNavigationUpdate(newRoute, oldRoute) {
 ...
 *
 * Note that the `$routeParams` are  updated *after* a route change completes successfully.
 ...
 *                  - Otherwise enable scrolling  if the `autoscroll` attribute value evaluated
...
describe('ngView', function() {
  ...
  describe('basics', function() {
    ...
    it('should be async even if served from cache', function() {
      ...
      inject(function($route, $rootScope, $location, $templateCache) {
        ...
        var called = 0;
        // we want to assert  during first watch
    ...
    it('should $destroy scope after update and reload',  function() {
      // this is a regression of bug, where $route doesn't copy scope when  updating
  ...
  describe('animations', function() {
    ...
    describe('autoscroll', function() {
      ...
      it('should  call $anchorScroll after the "enter" animation completes', inject(
        compileAndLink('<div><ng:view autoscroll></ng:view></div>'),
        function($rootScope, $location, $animate, $timeout) {
          $location.path('/foo');
...
describe('$route', function() {
  ...
  it('should change route even when  search param changes', function() {
    module(function($routeProvider) {
    ...
    describe('via `redirectTo`', function() {
      ...
      it('should replace the url when redirecting',  function() {
        ...
        inject(function($browser, $route, $location, $rootScope) {
          var $browserUrl = spyCallsWithArgs($browser, 'url').and.callThrough();
  ...
  describe('reloadOnSearch', function() {
    ...
    it('should reload when `reloadOnSearch` is false and url differs  in route path param',
      function() {
        var routeChange = jasmine.createSpy('route change');
