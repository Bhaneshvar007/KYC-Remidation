...
angular.module('ngCookies', ['ng']).
   ...
   * */
   provider('$cookies', [/** @this */function $CookiesProvider() {
     ...
     *
     * - **path** - `{string}` - The cookie will be available  for this path and its
     *   sub-paths. By default, this is the URL that appears in your `<base>` tag.
     * - **domain** - `{string}` - The cookie will be available  for this domain and
     *   its sub-domains. For security reasons the user agent will not accept the cookie
     *   if the current domain is not a sub-domain of this domain or equal to it.
     * - **expires** - `{string|Date}` - String of the form "Wdy, DD Mon YYYY HH:MM:SS GMT"
     *   or a Date object indicating the exact date/time this cookie will expire.
     * - **secure** - `{boolean}` - If `true`, then the cookie will  be available through a
     *   secured connection.
...
describe('$$cookieWriter', function() {
  ...
  describe('put via $$cookieWriter(cookieName, string), if no <base href> ', function() {
    ...
    it('should default path in cookie to "" (empty string)', function() {
      ...
      // This  fails in Safari and IE when cookiePath returns undefined
