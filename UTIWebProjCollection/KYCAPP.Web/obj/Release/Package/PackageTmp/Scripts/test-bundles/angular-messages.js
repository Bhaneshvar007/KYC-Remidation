 ...
 *
 * Currently, the ngMessages module  contains the code for the `ngMessages`, `ngMessagesInclude`
 ...
 * ### Message order
 *
 * By default, `ngMessages` will  display one message for a particular key/value collection at any time. If more
 ...
 *
 * While `ngMessages` will by default  display one error element at a time, the `ng-messages-multiple` attribute can
...
angular.module('ngMessages', [], function initAngularHelpers() {
   ...
   * on if they're present within the key/value object. By default,  one message will be displayed
  ...
  .directive('ngMessagesInclude',
    ...
    return {
      restrict: 'AE',
      require: '^^ngMessages', // we  require this for validation sake
            ...
            if (!currentElement) {
              $transclude(function(elm, newScope) {
                ...
                currentElement.on('$destroy', function() {
                  ...
                  // So this handler  handles cases where something else removed the message element.
...
describe('ngMessages', function() {
  ...
  // issue #12856
  it('should  detach the message object that is associated with the message node being removed',
    ...
    // There should  be the new message node
    expect(messageChildren(element).length).toEqual(1);
  ...
  it('should clean-up the ngMessage scope when a message is removed',
    inject(function($compile, $rootScope) {
      ...
      // We don't know exactly how many watchers are on the scope,  that there should be
      // one less now
