 ...
 * implementation for comm used browser apis that are hard to test, e.g. setTimeout, xhr,
 * cookies, etc.
 *
...
angular.mock.$Browser = function($log, $$taskTrackerFactory) {
   ...
   * You can request a specific type of tasks , by specifying a `taskType`.
   *
   ...
   * You can check for a specific type of tasks , by specifying a `taskType`.
   *
 ...
 *
 * Optionally, you can also pass a `delay` argument to  flush tasks that are scheduled to be
 * executed within `delay` milliseconds. Currently, `delay`  applies to timeouts, since all
  ...
  this.mode = function(mode) {
    ...
    switch (mode) {
      ...
      case 'rethrow':
        ...
        break;
      default:
        throw new Error('Unknown mode \'' + mode + '\',  \'log\'/\'rethrow\' modes are allowed!');
    }
  };
 ...
 * @example
 * !!!! WARNING !!!!!
 * This is not a complete Date object so  methods that were implemented can be called safely.
...
angular.mock.TzDate = function(offset, timestamp) {
  ...
  // provide this method  on browsers that already have it
  if (self.toISOString) {
...
function createHttpBackendMock($rootScope, $timeout, $delegate, $browser) {
   ...
   * @description
   * Creates a new backend definition that compares  with the requested route.
   *
   * @param {string} method HTTP method.
   ...
   * @description
   * Creates a new request expectation that compares  with the requested route.
   *
   * @param {string} method HTTP method.
   * @param {string} url HTTP url string that supports colon param matching.
   * @returns {requestHandler} Returns an object with `respond` method that controls how a matched
...
angular.mock.$TimeoutDecorator = ['$delegate', '$browser', function($delegate, $browser) {
   ...
   * sinceVersion="1.7.3"
   *
   * This method flushes all types of tasks (not  timeouts), which is unintuitive.
   ...
   * sinceVersion="1.7.3"
   *
   * This method takes all types of tasks (not  timeouts) into account, which is unintuitive.
   * It is recommended to use {@link ngMock.$verifyNoPendingTasks} instead, which additionally
   * allows checking for timeouts  (with `$verifyNoPendingTasks('$timeout')`).
   *
   * @description
   *
  ...
  $delegate.verifyNoPendingTasks = function() {
    ...
    if (pendingTasks.length) {
      ...
      var extraMessage = hasPendingTimeout ? '' : '\n\nNone of the pending tasks are timeouts. ' +
          'If you  want to verify pending timeouts, use ' +
 ...
 * Currently there is  one mock present in this module -
...
(function(jasmineOrMocha) {
   ...
   * *NOTE*: This function is declared  WHEN running tests with jasmine or mocha
   *
   ...
   *     // it loads  the "ng" module by default.
...
(function() {
   ...
   * @description
   *
   * This is a global (window) function that is  available when the {@link ngMock} module is
   * included.
   *
  ...
  window.browserTrigger = function browserTrigger(element, eventType, eventData) {
     ...
     * is  here to allow for testing where the timeStamp value is
     * read */
