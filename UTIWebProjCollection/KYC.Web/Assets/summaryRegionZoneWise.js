app.controller("summaryRegionZoneWise", ["$scope", "summaryRegionZoneWiseReportFactory", "$timeout", "$location", "$window", function ($scope, summaryRegionZoneWiseReportFactory, $timeout, $location, $window) {
    ...
    $scope.GetGridRecord = function (zone, region, ufc, from_date, to_date) {
        debugger;
        $scope.searchButtonText = "Searching";
        $scope.asOnDate = new Date(new Date().setHours(0, 0, 0, 0));

        summaryRegionZoneWiseReportFactory.getGridDetail(zone, region, ufc, from_date, to_date).then(function (success) {
            $scope.searchButtonText = "Search";
...
app.factory("summaryRegionZoneWiseReportFactory", ["$rootScope", "$http", "$q", "CommonFactory", function ($rootScope, $http, $q, CommonFactory) {
    ...
    this.getGridDetail = function (zone, region, ufc, from_date, to_date) {
        return $http.post(('/SummaryRegionZoneWiseRpt/GetGridDetail/'), { zone: zone, region: region, ufc: ufc, from_date: from_date, to_date: to_date });
    }
    return this;
}]);